...
def f(block, key):
    # print(f"block: {block.to_bytes(4,"big").hex().upper()}, key: {key.to_bytes(7, 'big').hex().upper()}")
...
def encode_block(block, derived_keys, encryption, number_of_rounds=16):
    block = permute(block, 64, INITIAL_PERMUTATION)
    block = block >> 32, block & 0xffffffff
    left = block[0].to_bytes(4, "big")
    right = block[1].to_bytes(4, "big")

    if not encryption:
        derived_keys = reversed(derived_keys)
    # Only 1 round
    for key in derived_keys[:number_of_rounds]:
        f_result = f(block[1], key)
        block = block[1], block[0] ^ f_result
